...
from PySide6.QtGui import QAction, QColor, QImage, QKeySequence, QPainter, QPen, QPixmap, QBrush, QWheelEvent
...
class ImageCanvas(QLabel):
    def __init__(self, parent=None):
        ...
        self.base_pixmap = None          # original image pixmap (never modified)
        self.display_pixmap = None       # pixmap scaled to fit label before zoom

        # --- fit-to-label scale (always recomputed from window size) ---
        self.fit_scale = 1.0

        # --- user zoom level (1.0 = fit to window) ---
        self.zoom_level = 1.0

        # Combined scale = fit_scale * zoom_level
        self.scale = 1.0

        # Pan offset in widget pixels (so the image can be panned when zoomed in)
        self.pan_offset_x = 0.0
        self.pan_offset_y = 0.0

        # The pixel offset inside the QLabel where the image starts drawing
        self.offset_x = 0
        self.offset_y = 0
    ...
    def set_image(self, qpixmap: QPixmap):
        ...
        # Reset zoom and pan whenever a new image is loaded
        self.zoom_level = 1.0
        self.pan_offset_x = 0.0
        self.pan_offset_y = 0.0
    ...
    # ------------------------------------------------------------------
    # Core rendering: compute scale + draw everything onto a canvas pixmap
    # ------------------------------------------------------------------
    def update_scaled_pixmap(self):
        ...
        # Scale that fits the image inside the label
        ...
        # Top-left corner of the image on the canvas (with pan)
    ...
    def apply_zoom(self, factor: float, anchor_widget_pos: QPointF = None):
        """Multiply zoom_level by factor, keeping anchor_widget_pos fixed."""
        ...
        if anchor_widget_pos is not None:
            # Compute image coordinate under the cursor before zoom change
            ...
            # Adjust pan so the same image pixel stays under the cursor
    ...
    def set_zoom_level(self, zoom: float, anchor_widget_pos: QPointF = None):
        """Set zoom to an absolute level."""
        if abs(self.zoom_level) < 1e-9:
            return
        factor = zoom / self.zoom_level
        self.apply_zoom(factor, anchor_widget_pos)
...
class RotationSlider(QWidget):
    """
    A horizontal slider that controls a live rotation preview angle.
    Dragging the slider rotates the displayed image in real time (no destructive
    write until the user confirms).  The image is re-rendered at the same zoom
    level so the canvas does NOT zoom out.
    """
    ...
    def __init__(self, parent=None):
        ...
        # Range: –180 … +180 degrees, stored as integer tenths of a degree
        # so slider int range is –1800 … +1800
        self._SCALE = 10
        ...
        self.slider.setTickInterval(150)       # every 15°
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.current_image = None           # numpy BGR array, always the original on disk
        ...
        # Rotation preview angle (from slider; not yet written to disk)
    ...
    # ------------------------------------------------------------------
    # Zoom helpers (delegated to canvas)
    # ------------------------------------------------------------------
    def zoom_in(self):
    ...
    def on_rotation_slider_changed(self, angle: float):
        """Called live as the user drags the slider. Re-renders the rotated
        image without writing anything to disk and without changing zoom."""
        self._preview_angle = angle
        self._refresh_canvas_with_preview()

    def _refresh_canvas_with_preview(self):
        """Rotate the in-memory image by _preview_angle and push it to the
        canvas without touching disk or zoom/pan state."""
        ...
        if abs(self._preview_angle) < 0.05:
            ...
            rotated = rotate_image_keep_size_crop_edges(self.current_image, self._preview_angle)
    ...
    def load_current_image(self):
        ...
        # Reset slider when loading a new image
        self._preview_angle = 0.0
    ...
    def update_status(self, extra: str = ""):
        shortcuts_text = (
            ...
            "Scroll=zoom | =/- keys=zoom | Z=zoom reset | Middle-drag=pan | "
