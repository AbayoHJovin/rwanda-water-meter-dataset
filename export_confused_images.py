...
Usage:
  python export_confused_images.py --weights best.pt --data data.yaml --output confused_images/
  python export_confused_images.py --weights best.pt --data data.yaml --iou-thresh 0.3   # looser match (recommended if labels are slightly off)
  python export_confused_images.py --weights best.pt --data data.yaml --debug            # prints per-image IoU matrix to spot double-detections
"""
...
COLOR_GT_MATCHED    = (34,  197,  94)
COLOR_GT_WRONG_CLS  = (0,   165, 255)
COLOR_GT_MISSED     = (60,  146, 251)
COLOR_PRED_CORRECT  = (21,  204, 250)
COLOR_PRED_WRONG    = (68,   68, 239)
COLOR_FP            = (0,     0, 200)
...
def nms(pred_boxes, iou_thresh=0.45):
    """
    Apply NMS across all predictions regardless of class.
    Keeps the highest-confidence box when two boxes overlap too much.
    This fixes the double-detection problem.
    """
    if not pred_boxes:
        return pred_boxes
...
def match_boxes(gt_boxes, pred_boxes, iou_thresh, debug=False, img_name=""):
    """
    Greedy bipartite matching by descending IoU.
    If debug=True, prints the full IoU matrix so you can see why boxes aren't matching.
    """
    if debug and gt_boxes and pred_boxes:
        print(f"\n  [{img_name}] IoU matrix (GT rows × Pred cols):")
        header = "  GT \\ Pred  " + "  ".join(
            f"P{j}({CLASS_NAMES[p['class']]}:{p['conf']:.2f})" for j, p in enumerate(pred_boxes)
        )
        print(f"  {header}")
        for i, gt in enumerate(gt_boxes):
            row = f"  G{i}({CLASS_NAMES[gt['class']]})    "
            row += "  ".join(f"{iou(gt['box'], p['box']):.2f}              "[: len(f"P{j}({CLASS_NAMES[p['class']]}:{p['conf']:.2f})")+2]
    ...
    if debug and (unmatched_gt or unmatched_pred):
        print(f"  Unmatched GT:   {[CLASS_NAMES[gt_boxes[i]['class']] for i in unmatched_gt]}")
        print(f"  Unmatched Pred: {[CLASS_NAMES[pred_boxes[j]['class']] for j in unmatched_pred]}")
        # Show best IoU each unmatched pred had with any GT
        for j in unmatched_pred:
            best = max((iou(gt['box'], pred_boxes[j]['box']), CLASS_NAMES[gt['class']])
                       for gt in gt_boxes) if gt_boxes else (0, "—")
            print(f"    Pred {j} ({CLASS_NAMES[pred_boxes[j]['class']]}) best IoU with any GT: "
                  f"{best[0]:.3f} (GT={best[1]}) — {'would match at iou_thresh=0.3' if best[0] >= 0.3 else 'truly no match'}")
...
def main():
    ...
    parser.add_argument("--iou-thresh", type=float, default=0.3,
                        help="IoU threshold for GT↔Pred matching. "
                             "Lower this (e.g. 0.3) if your labels are slightly off. "
                             "Default is 0.3 (was 0.5 before — that was too strict for meter digits).")
    parser.add_argument("--nms-iou",    type=float, default=0.45,
                        help="IoU threshold for NMS deduplication of predictions. "
                             "Lower = more aggressive dedup. 0.45 recommended for digits.")
    parser.add_argument("--split",      type=str, default="val",
                        choices=["train", "val", "test"])
    parser.add_argument("--debug",      action="store_true",
                        help="Print per-image IoU matrix to diagnose matching failures")
    args = parser.parse_args()
    ...
    image_files = sorted(list(image_dir.glob("*.jpg")) + list(image_dir.glob("*.png")))
    print(f"Found {len(image_files)} images in '{args.split}' split")
    print(f"GT↔Pred IoU threshold : {args.iou_thresh}  (use --debug to inspect mismatches)")
    print(f"NMS IoU threshold     : {args.nms_iou}  (deduplicates overlapping predictions)\n")
    ...
    for idx, img_path in enumerate(image_files):
        ...
        results    = model.predict(str(img_path), conf=args.conf,
                                   iou=args.nms_iou,   # ← tighter NMS here
                                   verbose=False)[0]
        pred_boxes = []
        if results.boxes is not None:
            for box in results.boxes:
        ...
        # Extra NMS pass across all classes (YOLO's built-in NMS is per-class,
        # so two different class predictions on the same digit can both survive)
        pred_boxes = nms(pred_boxes, iou_thresh=args.nms_iou)

        gt_to_pred, pred_to_gt, unmatched_gt, unmatched_pred = \
            match_boxes(gt_boxes, pred_boxes, args.iou_thresh,
                        debug=args.debug, img_name=img_path.name)
    ...
    with open(summary_path, 'w') as f:
        ...
        f.write(f"Split:          {args.split}\n")
        f.write(f"Conf:           {args.conf}\n")
        f.write(f"IoU match:      {args.iou_thresh}\n")
        f.write(f"NMS IoU:        {args.nms_iou}\n\n")
        f.write(f"False Negatives: {stats['false_negatives']}\n")
        f.write(f"False Positives: {stats['false_positives']}\n")
        f.write(f"Misclassified:   {stats['misclassified']}\n\n")
